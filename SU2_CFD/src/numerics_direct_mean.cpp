...
*/
  ...
  if (implicit) {
      ...
      dmRdj[0] = -mR/Density_j;
  	  ...
  	  for (iDim=0; iDim < nDim; iDim++) {
  	  	dmLdi[iDim + 1] = UnitNormal[iDim]/(Density_i*SoundSpeed_i);
  	    dmRdj[iDim + 1] = UnitNormal[iDim]/(Density_j*SoundSpeed_j);
  	  }
 	  dmLdi[nVar-1] =  0;
      dmRdj[nVar-1] =  0;
 	...
 	// adding dPressure_i/dui * m+   *a, dPressure_j/duj * m-   *a
  	...
  	// d[0 0 0 0 P_j]/uj = !=0 only in row nVar, Temp_Vector2 = row[nVar]
  	  ...
  	  Temp_Vector1[0] = Chi_i-StaticEnergy_i*Kappa_i/Density_i + 0.5*Kappa_i * sq_vel_i/Density_i;
  	  Temp_Vector2[0] = Chi_j-StaticEnergy_j*Kappa_j/Density_j + 0.5*Kappa_j * sq_vel_j/Density_j;
...
CUpwGeneralAUSM_Flow::CUpwGeneralAUSM_Flow(unsigned short val_nDim, unsigned short val_nVar, CConfig *config) : CNumerics(val_nDim, val_nVar, config) {
...
CUpwGeneralAUSM_Flow::~CUpwGeneralAUSM_Flow(void) {
...
void CUpwGeneralAUSM_Flow::ComputeResidual(double *val_residual, double **val_Jacobian_i, double **val_Jacobian_j, CConfig *config) {
